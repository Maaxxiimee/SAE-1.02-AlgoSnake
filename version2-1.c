...
// int lesPavesX[NB_PAVES] = { 3, 3, 34, 34, 21, 15};
...
void choisirDirection(tPlateau plateau,int lesX[], int lesY[], char *direction, int nbPommes, char chemin[20], int destX, int destY){
	...
	if (lesY[0] < destY) { // direction BAS
		...
		for (int i = 1; i < TAILLE; i++){
			...
			if (sqrt(pow((lesX[0]-lesX[i]),2) + pow((lesY[0]-lesY[i]),2)) < i && lesY[i] < lesY[0] && lesX[i] == lesX[0] && !vaSeToucher){
				peutAller = false;
			}
		}
		if (!vaSeToucher && peutAller){
		...
		else if (lesX[0] > destX && plateau[lesX[0]-1][lesY[0]] != BORDURE && feuVertGauche){
			*direction = GAUCHE;
		}
		else if (lesX[0] < destX && plateau[lesX[0]+1][lesY[0]] != BORDURE && feuVertDroit){
			*direction = DROITE;
		}
		else{
			*direction = BAS;
...
void progresser(int lesX[], int lesY[], char direction, tPlateau plateau, bool * collision, bool * pomme, int nbPommes, char chemin[20]){
	...
	*pomme = false;
	...
	// Teleportation 
	if (lesX[0] == LARGEUR_PLATEAU/2 && lesY[0] == 0 && direction == HAUT){ // Haut
		lesY[0] = HAUTEUR_PLATEAU+1;  // Téléportation de la tête
		strcpy(chemin, checkMeilleurChemin(lesX, lesY, nbPommes));
	}
	else if (lesX[0] == LARGEUR_PLATEAU/2 && lesY[0] == HAUTEUR_PLATEAU+1 && direction == BAS){ // Bas
		lesY[0] = 1;
		strcpy(chemin, checkMeilleurChemin(lesX, lesY, nbPommes));
		
	}
	else if (lesX[0] == LARGEUR_PLATEAU+1 && lesY[0] == HAUTEUR_PLATEAU/2 && direction == DROITE){ // Droite
		lesX[0] = 1;
		strcpy(chemin, checkMeilleurChemin(lesX, lesY, nbPommes));
		
	}
	else if (lesX[0] == 0 && lesY[0] == HAUTEUR_PLATEAU/2 && direction == GAUCHE){ // Gauche
